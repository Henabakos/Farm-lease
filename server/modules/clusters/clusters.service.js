...
function toDto(c) {
  ...
  return {
    ...
    has_verified_survey: Boolean(c._verified),
...
export async function list(query, viewer) {
      ...
      isAdmin(viewer)
        ...
        : {
            OR: [
              { status: 'ACTIVE' },
              { ownerId: viewer.id },
              { memberships: { some: { userId: viewer.id, isActive: true } } },
            ],
          },
      status ? { status } : {},
      ownerId ? { ownerId } : {},
  ...
  const [rows, total] = await Promise.all([
    prisma.cluster.findMany({
      ...
      include: {
        ...
        boundaries: { select: { verificationStatus: true }, take: 1, orderBy: { createdAt: 'desc' } },
      },
    }),
  ...
  return paginated(
    rows.map((r) =>
      toDto({
        ...r,
        _verified: r.boundaries.some((b) => b.verificationStatus === 'VERIFIED'),
...
export async function getById(id, _viewer) {
  ...
  return toDto({
    ...c,
    _verified: c.boundaries?.some((b) => b.verificationStatus === 'VERIFIED'),
...
export async function create(input, viewer) {
  ...
  const cluster = await prisma.$transaction(async (tx) => {
    ...
    await tx.clusterMembership.create({
      data: { userId: viewer.id, clusterId: created.id, role: 'REPRESENTATIVE' },
    });
    await recordOutbox(tx, {
      eventType: 'cluster.created',
      aggregateType: 'Cluster',
      aggregateId: created.id,
      payload: { clusterId: created.id, ownerId: viewer.id, name: created.name },
    });
    return created;
  });
...
export async function remove(id, viewer) {
  ...
  await prisma.cluster.update({ where: { id }, data: { status: 'ARCHIVED' } });
...
export async function inviteMember(clusterId, email, role, viewer) {
  ...
  return {
    ...
    member: {
      ...
      joined_at: membership.joinedAt?.toISOString?.() ?? membership.joinedAt,
    },
  };
}
...
export async function updateMemberRole(clusterId, userId, role, viewer) {
  ...
  if (!membership.isActive) throw new ForbiddenError('Cannot update role for inactive member');
...
export async function adminVerify(clusterId, viewer) {
  if (!isAdmin(viewer)) throw new ForbiddenError('Only admins can verify clusters');
  const cluster = await loadOrThrow(clusterId);
  ...
  if (latestBoundary) {
    ...
    const md = { ...(cluster.metadata || {}), is_verified: true, verified_by: viewer.id, verified_at: new Date().toISOString() };
