    ...
    ...(q
      ? {
          OR: [
            ...
            ...(isAdmin(viewer) ? [{ email: { contains: q, mode: 'insensitive' } }] : []),
          ],
        }
      : {}),
  };
  const [rows, total] = await Promise.all([
    prisma.user.findMany({ where, ...paginate({ page, pageSize }), orderBy: { createdAt: 'desc' } }),
...
export async function verify({ targetId, viewer }) {
  if (!isAdmin(viewer)) throw new ForbiddenError('Only admins can verify users');
