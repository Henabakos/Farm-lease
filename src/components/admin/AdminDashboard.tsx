    ...
    >
      <motion.div variants={item} className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="gap-2 h-10 px-4 rounded-md border-slate-200 bg-white hover:bg-slate-50 hover:text-primary transition-all">
            ...
            <Card className="border border-slate-200 shadow-sm bg-white hover:shadow-md transition-all duration-300 group overflow-hidden relative rounded-lg">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div className={cn("w-10 h-10 rounded-md flex items-center justify-center transition-transform duration-300 group-hover:scale-105", stat.bg, stat.color)}>
                    <stat.icon className="w-5 h-5" />
                  </div>
                  <Badge variant="secondary" className="bg-slate-100 text-slate-600 border-none font-semibold px-2 py-0.5 rounded-md text-[10px]">
                    {stat.change}
                  </Badge>
                </div>
                <div className="mt-4">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">{stat.title}</p>
                  <h3 className="text-xl font-bold mt-1 tracking-tight text-foreground">{stat.value}</h3>
                </div>
              </CardContent>
            </Card>
          </motion.div>
            ...
            <CardHeader className="p-6 pb-0">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <CardDescription className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Manage and monitor all system entities and users.</CardDescription>
                </div>
                  ...
                  {activeTab === 'CLUSTERS' && (
                    <div className="space-y-3">
                      {clusters.map((cluster) => (
                        <div key={cluster.id} className="flex items-center justify-between p-4 rounded-md bg-slate-50 border border-slate-200 hover:border-slate-300 hover:bg-slate-100 transition-all group">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-bold text-sm text-slate-900 tracking-tight">{cluster.name}</h4>
                              <p className="text-[11px] text-slate-500 font-medium">{cluster.location}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            {cluster.isVerified ? (
                              <Badge className="bg-emerald-50 text-emerald-600 border border-emerald-100 font-bold px-2 py-0.5 rounded-md text-[9px] uppercase tracking-wider">Verified</Badge>
                            ) : (
                              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 font-bold rounded-md h-8 text-[10px] uppercase tracking-wider px-3" onClick={() => handleVerifyCluster(cluster.id)}>
                                Verify Now
                              </Button>
                            )}
                  ...
                  {activeTab === 'PAYMENTS' && (
                    <div className="space-y-3">
                      {payments.map((payment) => (
                        <div key={payment.id} className="flex items-center justify-between p-4 rounded-md bg-slate-50 border border-slate-200 hover:border-slate-300 hover:bg-slate-100 transition-all group">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-bold text-sm text-slate-900 tracking-tight">${payment.amount.toLocaleString()}</h4>
                              <p className="text-[11px] text-slate-500 font-medium">{payment.agreementTitle}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <Badge className={cn(
                              ...
                              payment.status === 'SUBMITTED' ? "bg-amber-50 text-amber-600 border-amber-100" : "bg-slate-50 text-slate-600 border-slate-200"
                            )}>
                              {payment.status}
                            </Badge>
                            {payment.status === 'SUBMITTED' && (
                              <Button size="sm" className="bg-primary hover:bg-primary/90 font-bold rounded-md h-8 text-[10px] uppercase tracking-wider px-3" onClick={() => handleVerifyPayment(payment.id)}>
                                Verify
                              </Button>
                            )}
                          </div>
          ...
          <motion.div variants={item}>
            <Card className="border border-slate-200 shadow-sm bg-white rounded-lg overflow-hidden">
              <CardHeader className="p-6">
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-1">Critical items requiring admin approval.</CardDescription>
              </CardHeader>
              <CardContent className="p-6 pt-0 space-y-3">
                {pendingClusters.slice(0, 2).map((cluster) => (
                  <div key={cluster.id} className="p-4 rounded-md bg-slate-50 border border-slate-200 space-y-3 group">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-xs text-slate-900 tracking-tight">{cluster.name}</h4>
                          <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">{cluster.location}</p>
                        </div>
                      </div>
                      <Button size="icon" variant="ghost" className="h-7 w-7 rounded-md text-emerald-600 hover:bg-white border border-transparent hover:border-slate-200" onClick={() => handleVerifyCluster(cluster.id)}>
                ...
                {pendingPayments.slice(0, 2).map((payment) => (
                  <div key={payment.id} className="p-4 rounded-md bg-slate-50 border border-slate-200 space-y-3 group">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-xs text-slate-900 tracking-tight">${payment.amount.toLocaleString()}</h4>
                          <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">{payment.agreementTitle}</p>
                        </div>
                      </div>
                      <Button size="icon" variant="ghost" className="h-7 w-7 rounded-md text-amber-600 hover:bg-white border border-transparent hover:border-slate-200" onClick={() => handleVerifyPayment(payment.id)}>
                        <CheckCircle2 className="w-3.5 h-3.5" />
                      </Button>
                    </div>
                  </div>
                ))}
                <Button variant="ghost" className="w-full h-10 rounded-md font-bold text-[10px] uppercase tracking-wider text-primary hover:bg-slate-50 border border-transparent hover:border-slate-200">
                  View Full Queue
            ...
            <Card className="border border-slate-200 shadow-sm bg-slate-50 rounded-lg overflow-hidden">
              <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="font-bold text-sm tracking-tight text-slate-900">Security Status</h3>
                    <p className="text-[10px] font-bold text-primary uppercase tracking-wider">All Systems Secure</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[10px] text-slate-500 uppercase tracking-wider">Firewall</span>
                    <Badge className="bg-emerald-500 text-white border-none font-bold text-[8px] uppercase tracking-wider px-1.5 py-0 rounded-md">Active</Badge>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[10px] text-slate-500 uppercase tracking-wider">Encryption</span>
                    <Badge className="bg-emerald-500 text-white border-none font-bold text-[8px] uppercase tracking-wider px-1.5 py-0 rounded-md">AES-256</Badge>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-[10px] text-slate-500 uppercase tracking-wider">Audit Logging</span>
                    <Badge className="bg-emerald-500 text-white border-none font-bold text-[8px] uppercase tracking-wider px-1.5 py-0 rounded-md">Enabled</Badge>
                  </div>
                </div>
