...
export function AdminPaymentView({
  ...
  const openStatusDialog = (payment: Payment, status: PaymentStatus) => {
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage and review all payment transactions</p>
        </div>
      </div>
      ...
      <motion.div variants={container} className="grid grid-cols-1 gap-4">
        {filteredPayments.length === 0 ? (
          <motion.div variants={item} className="rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
            ...
            <h2 className="text-base font-bold tracking-tight text-slate-900">No payments found</h2>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => openStatusDialog(payment, 'PENDING')}>
                              <Clock className="w-4 h-4 mr-2" />
                              Set to Pending
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => openStatusDialog(payment, 'SUBMITTED')}>
                              <Upload className="w-4 h-4 mr-2" />
                              Set to Submitted
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => openStatusDialog(payment, 'VERIFIED')}>
                              <CheckCircle2 className="w-4 h-4 mr-2" />
                              Set to Verified
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => openStatusDialog(payment, 'REJECTED')}>
                              <XCircle className="w-4 h-4 mr-2" />
                              Set to Rejected
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => openDeleteDialog(payment)} className="text-red-600">
      ...
      <Dialog open={statusDialogOpen} onOpenChange={setStatusDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Change the status of payment {selectedPayment?.id.toUpperCase()} from {selectedPayment?.status} to {newStatus}.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will soft delete payment {selectedPayment?.id.toUpperCase()}. The payment will be hidden from lists but retained in the database with an audit trail.
            </DialogDescription>
          </DialogHeader>
          <Textarea
